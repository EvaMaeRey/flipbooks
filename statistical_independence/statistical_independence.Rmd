...
date: "September 21 and Oct 24, 2019"
output:
  # html_document:
  #   toc: no
...
knitr::opts_chunk$set(out.width = "10%", fig.height = 4, comment = "")
```
...
- When you have *evidence* of statistical *dependence*.  The probability of observing of observing such a strong relationship --- or stronger --- under *statistical independence* (the null) is quite small.  
...
- Comparison will be made with expectation about null (no relationship).  Such a strong relationship is unlikely to be observed by chance in a sample, if there is *not* dependence.  
...
cor.test(x = my_data$tail_length, 
              y = my_data$brain_weight) #REVEAL
```

---

`r apply_reveal("continuous0", user_reveal_defined = T)`
...
`r apply_repeat_reveal("continuous")`
...
`r apply_repeat_reveal(chunk_name = "c_and_d2")`
...
`r apply_repeat_reveal(chunk_name = "c_and_d3")`
...
`r apply_repeat_reveal(chunk_name = "d_and_d")`
...
`r apply_repeat_reveal(chunk_name = "lady_tea")`
...
`r apply_repeat_reveal(chunk_name = "d4_d2")`
