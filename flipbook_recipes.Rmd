...
### Let's build a *new* source code chunk, "my_mtcars", then *only show one panel* --- the output.  
...
#### Then we use `chunk_reveal()` on this the "my_mtcars" code chunk.  Using the display_type = "output" will give us *only the output panel*. 
...
# Source for a tiny flipbook and output

## You might find it useful to also look at an .Rmd and the resultant slide show.

##- [.Rmd](EvaMaeRey/flipbookr/flipbooks/tiny_flipbook.Rmd)
...
## But the .html slides that {xaringan} produces aren't contained in a single file, so doing so can be perilous! 
...
## For more on this topic and also more flipbook recipes consult template that travels with flipbookr.  The rendered output is [here](https://evamaerey.github.io/flipbooks/flipbookr/skeleton#220).
